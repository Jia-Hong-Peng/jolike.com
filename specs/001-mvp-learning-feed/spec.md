...
### User Story 2 — 學習卡片互動：播放、跟讀 (Priority: P2)

使用者在學習 Feed 中看到一張卡片，可以重播影片片段，並進行跟讀（錄音後聆聽自己的聲音）。

**Why this priority**: 核心學習行為。看懂 + 說出來，是 JoLike English 與一般字典 app 的最大差異。

**Independent Test**: 可以獨立測試：在顯示單張卡片的測試頁面上，驗證兩個操作（重播、跟讀）是否可正常執行。

**Acceptance Scenarios**:

1. **Given** 使用者看到一張卡片，**When** 點擊重播按鈕，**Then** 影片片段從頭播放一次。
2. **Given** 使用者點擊跟讀按鈕，**When** 系統播放句子後，**Then** 錄音自動開始（最多 10 秒），錄音結束後立即播放使用者的聲音。
3. **Given** 使用者跟讀後播放自己的聲音，**When** 播放結束，**Then** 跟讀按鈕恢復初始狀態，使用者可再次跟讀。

> **Scope Note (SCOPE REDUCTION)**: 慢速播放（0.7x）從 MVP 中移除，作為 v1.1 功能。

---

### User Story 3 — 卡片互動：會了 / 不熟 (Priority: P2)

使用者對每張卡片標記學習狀態，並可滑動到下一張卡片。

**Why this priority**: 讓使用者有主控感，且為後續個人化 Feed 提供資料基礎（即使 MVP 不做個人化）。

**Independent Test**: 可以獨立測試：點擊兩個按鈕後，驗證對應狀態是否在當次 session 中正確記錄，且可滑動到下一張。

**Acceptance Scenarios**:

1. **Given** 使用者看到一張卡片，**When** 點擊「我會了」，**Then** 卡片淡出，自動滑到下一張；該卡片狀態記錄為「已掌握」。
2. **Given** 使用者看到一張卡片，**When** 點擊「不熟」，**Then** 卡片狀態記錄為「不熟」，滑到下一張。
3. **Given** 使用者上滑卡片，**When** 滑動距離超過閾值，**Then** 切換到下一張卡片（等同「我會了」）。

> **Scope Note (SCOPE REDUCTION)**: 收藏功能從 MVP 中移除，作為 v1.1 功能。不熟卡片不做 re-queue 邏輯（MVP 只記錄狀態）。

---

### ~~User Story 4 — 學習 Session 完成~~ (CUT — v1.1 Backlog)

> **SCOPE REDUCTION**: 此 User Story 從 MVP 中完整移除。完成所有卡片後，顯示一個簡單的「換影片」按鈕即可，不做統計摘要畫面。
>
> **Rationale**: 核心假設驗證不需要摘要功能。優先確認 US1（URL→卡片）的核心流程可行，再做 retention 機制。
...
- **FR-005**: 系統 MUST 支援重播（1x）
- **FR-006**: 系統 MUST 支援跟讀：播放句子 → 錄音 → 播放使用者錄音，整個流程不需離開當前卡片
- **FR-007**: 系統 MUST 支援兩種互動標記：會了 / 不熟，並在當次 session 中保留狀態（localStorage）
...
- 字幕來源為 YouTube timedtext 公開端點（英文），不使用 YouTube Data API v3（需 API Key）
- NLP 分析（關鍵字提取）使用 `compromise.js` + COCA 5000 詞頻表 + CC-CEDICT 字典，**在瀏覽器端執行**，零 API 成本
- 後端（Cloudflare Pages Functions）只負責 YouTube timedtext proxy + 原始字幕快取（Cloudflare D1）
- 不需要使用者帳號，session 互動狀態（會了/不熟）暫存於 localStorage
- 跟讀功能需要使用者主動授予麥克風權限（Web API）
- 學習卡片由前端從原始字幕分析生成，不持久化到後端資料庫
