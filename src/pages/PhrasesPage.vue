<template>
  <div class="min-h-screen bg-black text-white flex flex-col">
    ...
    <header class="flex items-center justify-between px-4 py-3 border-b border-gray-800">
      <button class="text-gray-400 min-h-[44px] min-w-[44px] flex items-center" @click="goBack">
        ← 返回
      </button>
    ...
    <div v-if="loading" class="flex-1 flex items-center justify-center">
      <div class="space-y-3 w-full max-w-lg px-4">
        <div
v-for="i in 8" :key="i"
          class="h-16 bg-gray-800 rounded-xl animate-pulse"
></div>
      </div>
    </div>
    ...
    <div
v-else-if="!loading && phrases.length === 0"
...
>
    ...
    <main v-else class="flex-1 overflow-y-auto">
      <ul class="divide-y divide-gray-800/50">
        <li
v-for="(item, idx) in phrases" :key="item.phrase"
...
>
              ...
              <div v-if="item.example_video_id" class="flex gap-2 mt-3">
                <!-- Play in YouTube -->
                <a
:href="youtubeUrl(item)" target="_blank" rel="noopener"
