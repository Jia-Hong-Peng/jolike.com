...
| `/vocab-stats/` | VocabStatsPage — SRS 個人化詞彙課表（排行榜即課程表）|
...
#### 詞彙索引批次補建（scripts/batch-vocab-index.mjs）

繞過 Worker 10ms CPU 限制，在本機批次對所有影片重建 `video_vocab` 索引：

```bash
# 跑全部影片
API_BASE=https://jolike.com node scripts/batch-vocab-index.mjs

# 只跑指定頻道，最多 50 部
API_BASE=https://jolike.com node scripts/batch-vocab-index.mjs --limit 50 --channel UCxxxxxx
```

#### 字幕抓取（scripts/fetch-transcripts.py）
